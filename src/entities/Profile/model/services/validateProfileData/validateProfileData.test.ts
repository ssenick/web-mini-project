...
describe('validateProfileData.test', () => {
   ...
   test(' name not entered', () => {
      ...
      expect(result).toEqual([ValidateProfileErrors.INCORRECT_USER_FIRST_NAME]);
   });
   test(' name not entered', () => {
      const result = validateProfileData({ ...data, username: '' });
      expect(result).toEqual([ValidateProfileErrors.INCORRECT_USER_USER_NAME]);
   });
   test(' lastname not entered', () => {
      const result = validateProfileData({ ...data, lastname: '' });
      expect(result).toEqual([ValidateProfileErrors.INCORRECT_USER_LAST_NAME]);
   });
